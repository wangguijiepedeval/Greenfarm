...
<script>
import { listCart, getCart, delCart, addCart, updateCart } from "@/api/farm/cart";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除cart管理编号为"' + userids + '"的数据项？').then(function() {
        return delCart(userids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('farm/cart/export', {
        ...this.queryParams
