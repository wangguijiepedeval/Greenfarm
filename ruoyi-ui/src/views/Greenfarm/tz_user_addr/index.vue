    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['Greenfarm:tz_user_addr:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['Greenfarm:tz_user_addr:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['Greenfarm:tz_user_addr:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['Greenfarm:tz_user_addr:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="tz_user_addrList" @selection-change="handleSelectionChange">
...
<script>
import { listTz_user_addr, getTz_user_addr, delTz_user_addr, addTz_user_addr, updateTz_user_addr } from "@/api/Greenfarm/tz_user_addr";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        status: [
          { required: true, message: "状态,1正常，0无效不能为空", trigger: "blur" }
        ],
        commonAddr: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTz_user_addr(this.queryParams).then(response => {
        this.tz_user_addrList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTz_user_addr(addrId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.addrId != null) {
            updateTz_user_addr(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTz_user_addr(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户配送地址编号为"' + addrIds + '"的数据项？').then(function() {
        return delTz_user_addr(addrIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('Greenfarm/tz_user_addr/export', {
        ...this.queryParams
      }, `tz_user_addr_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
